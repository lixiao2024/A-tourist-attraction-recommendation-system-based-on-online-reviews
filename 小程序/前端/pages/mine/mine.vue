...
<script>
	import { getUserStats, refreshUserLikes } from '@/request/api.js'
	export default {
		data() {
			return {
				isLogged: false,
				...
				tempAvatarUrl: '',
				tempNickName: '',
				statsRefreshTimer: null // 统计数据刷新定时器
			}
		},
		onLoad() {
			...
			// 设置定时器，每60秒刷新一次用户统计数据
			this.statsRefreshTimer = setInterval(() => {
				if (this.isLogged) {
					console.log('定时刷新用户统计数据...');
					this.loadUserStatsData();
				}
			}, 60000); // 每60秒刷新一次
		...
		methods: {
			...
			loadUserStatsData() {
				...
				getUserStats().then(data => {
					console.log('获取用户统计数据成功:', data);
					
					// 更新统计数据
					const { likes_received, fans_count, follow_count, favorites_count } = data;
					
					// 更新页面显示的统计数据
					this.statsData = [
						{ label: '粉丝', value: fans_count, icon: 'person' },
						{ label: '获赞', value: likes_received, icon: 'heart' },
						{ label: '关注', value: follow_count, icon: 'eye' },
						{ label: '收藏', value: favorites_count, icon: 'star' }
					];
			...
			// 刷新用户获赞数
			refreshUserLikes() {
				console.log('开始刷新用户获赞数...');
				// 检查是否已登录
				const token = uni.getStorageSync('token');
				
				if (!token) {
					console.log('用户未登录，无法刷新获赞数');
					return;
				}
				
				// 调用API函数刷新用户获赞数
				refreshUserLikes().then(res => {
					console.log('刷新用户获赞数成功:', res);
					// 刷新成功后立即更新统计数据
					this.loadUserStatsData();
				}).catch(err => {
					console.error('刷新用户获赞数失败:', err);
				});
			}
		}
	}
