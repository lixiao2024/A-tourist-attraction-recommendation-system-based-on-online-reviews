...
# 打开 CSV 文件并准备写入
with open('comments_bada_ling.csv', mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['business_id', 'content', 'score'])  # 写入 CSV 文件的表头（字段名）

    # 循环翻页抓取评论（假设每页10条评论，爬取5页）
    for page in range(1, 10):  # 假设只爬取前5页
...
print("所有评论已成功保存到 'comments_bada_ling.csv' 文件中！")
